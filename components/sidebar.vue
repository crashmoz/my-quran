<template>
	<v-list density="compact" nav>
		<v-list-item v-for="(item, i) in surah.data" :key="i" :value="item" color="green-darken-2" link
			:to="`/surah/${item.nomor}`">
			<template v-slot:prepend>
				<div class="d-flex justify-center align-center text-amber-accent-2 mr-4">
					<svg width="28.5" height="32.25" viewBox="0 0 38 43" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path
					...
					<span style="position: absolute; font-family: Quicksand; font-size: 14px; font-weight: 600;"
						class="text-green-darken-2">{{ item.nomor }}</span>
				</div>
				<!-- <v-icon>iconName</v-icon> -->
			</template>
			<v-list-item-title v-text="item.namaLatin" />
			<v-list-item-subtitle v-text="`${item.jumlahAyat} Ayat`"></v-list-item-subtitle>
		</v-list-item>
	</v-list>
</template>
